...
//
// This file implements the TTK sprite sheet quad renderer
// (Note that this is far from a shippable solution, only provided for simplicity)
//
...
TTK::SpriteSheetQuad::SpriteSheetQuad()
{
	...
	const char* fsSource = R"LIT(#version 440
            layout(binding = 0, location = 1) uniform sampler2D xSampler;
...
void TTK::SpriteSheetQuad::SliceSpriteSheet(const char* fileName, int numSpritesPerRow, int numRows, float animTime)
{
	...
	float spriteWidth = m_Texture.GetWidth() / numSpritesPerRow;
	float spriteHeight = m_Texture.GetHeight() / numRows;
...
void TTK::SpriteSheetQuad::Update(float deltaTime) {
	...
	if (m_FrameTime > m_FrameLength[m_CurrentFrame]) {
		...
		m_CurrentFrame++;
		if (m_DoesLoop)
			m_CurrentFrame = m_CurrentFrame % m_FrameLength.size();
		else
			m_CurrentFrame = m_CurrentFrame > m_FrameLength.size() - 1 ? m_FrameLength.size() - 1 : m_CurrentFrame;
	}
}
